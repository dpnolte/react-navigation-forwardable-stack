...
class StackViewLayout extends React.Component<Props, State> {
  ...
  private transitionConfig: (HeaderTransitionConfig & TransitionConfig) | undefined;
  ...
  private renderHeader(scene: Scene, headerMode: HeaderMode) {
    ...
    const renderHeader = header || ((props: HeaderProps) => <Header {...(props as any)} />);
  ...
  private goBack(backFromIndex: number, duration: number) {
    ...
    if (backFromIndex === toValue) return;
  ...
  private goForward(forwardFromIndex: number, duration: number) {
    const { navigation, position, scenes } = this.props.transitionProps;
    const toValue = Math.min(forwardFromIndex + 1, scenes.length - 1);
    if (toValue === forwardFromIndex) {
      return;
    }
    ...
    const onCompleteAnimation = () => {
      this.immediateIndex = null;
      const goToScene = scenes.find((s) => s.index === toValue);
      if (goToScene) {
        (goToScene.route as any).isNextRoute = false;
        navigation.dispatch(
      ...
      >
        ...
        >
          <StackGestureContext.Provider value={this.panGestureRef}>
            <ScreenContainer style={styles.scenes}>{scenes.map(this.renderCard)}</ScreenContainer>
            {floatingHeader}
  ...
  private gestureActivationCriteria() {
    ...
    if (this.isMotionVertical()) {
      const height: number = (layout.height as any).__getValue();
    ...
    // const width: number = (layout.width as any).__getValue();
  ...
  private isGestureEnabled() {
    ...
    return typeof gesturesEnabled === 'boolean' ? gesturesEnabled : true;
  }
  ...
  private computeHorizontalGestureValue({ translationX }: { translationX: number }) {
    const {
      transitionProps: { navigation, layout, hasNextScene, hasPreviousScene },
    } = this.props;
    const { index } = navigation.state;

    // TODO: remove this __getValue!
    const distance: number = (layout.width as any).__getValue();

    const x = this.isMotionInverted() ? -1 * translationX : translationX;

    const value = index - x / distance;
    const result = clamp(
      hasPreviousScene ? index - 1 : index,
      value,
      hasNextScene ? index + 1 : index
    );
    return result;
  }

  private computeVerticalGestureValue({ translationY }: { translationY: number }) {
    const {
    ...
    const distance: number = (layout.height as any).__getValue();
  ...
  private handlePanGestureStateChange = ({ nativeEvent }: PanGestureHandlerGestureEvent) => {
    if ((nativeEvent as any).oldState === GestureState.ACTIVE) {
      // Gesture was cancelled! For example, some navigation state update
      // arrived while the gesture was active that cancelled it out
      if ((this.positionSwitch as any).__getValue() === 1) {
        return;
      }
      ...
      if (this.props.onGestureBegin) this.props.onGestureBegin();
  ...
  private prepareGesture() {
    ...
    if (
      (this.props.transitionProps.layout.width as any).__getValue() === 0 ||
      (this.props.transitionProps.layout.height as any).__getValue() === 0
    ) {
      return;
    }
  ...
  private prepareGestureHorizontal() {
    const { navigation, hasNextScene, hasPreviousScene } = this.props.transitionProps;
    const { index } = navigation.state;
    ...
    if (this.isMotionInverted()) {
      ...
      if (hasPreviousScene && hasNextScene) {
        this.gesturePosition = Animated.add(
          ...
          outputRange: [index - 1, index, index + 1],
          extrapolate: 'clamp',
        });
      } else if (!hasPreviousScene && hasNextScene) {
        this.gesturePosition = Animated.add(
          index,
          Animated.multiply(
            -1,
  ...
  private handleReleaseHorizontal(
    ...
    const {
      transitionProps: { navigation, position, layout, hasNextScene, hasPreviousScene },
    } = this.props;
    const { index } = navigation.state;
    const immediateIndex = this.immediateIndex == null ? index : this.immediateIndex;

    // Calculate animate duration according to gesture speed and moved distance
    const distance = (layout.width as any).__getValue();
    ...
    if (gestureVelocity < -50) {
      ...
      this.goForward(immediateIndex, resetDuration);
      return;
    }
    if (gestureVelocity > 50) {
      if (this.props.onGestureEnd) this.props.onGestureEnd();
    ...
    if (hasPreviousScene && value <= index - POSITION_THRESHOLD) {
      if (this.props.onGestureEnd) this.props.onGestureEnd();
      this.goBack(immediateIndex, goBackDuration);
    } else if (hasNextScene && value > index + POSITION_THRESHOLD) {
      this.goForward(immediateIndex, goBackDuration);
      if (this.props.onGestureEnd) this.props.onGestureEnd();
    } else {
  ...
  private handleReleaseVertical(
    ...
    // Calculate animate duration according to gesture speed and moved distance
    const distance = (layout.height as any).__getValue();
    const isMotionInverted = this.isMotionInverted();
  ...
  private getHeaderBackTitleVisible() {
    ...
    const enabledByDefault = !(layoutPreset === 'left' || Platform.OS !== 'ios');
  ...
  private prepareTransitionConfig() {
    this.transitionConfig = StackViewTransitionConfigs.getTransitionConfig(
  ...
  private preparePosition() {
    if (this.gesturePosition) {
      ...
      this.position = Animated.add(
        Animated.multiply(this.props.transitionProps.position, this.positionSwitch),
  ...
  private renderCard = (scene: Scene) => {
    ...
    return (
      <StackViewCard
        ...
        scene={scene}
        focusedIndex={this.props.transitionProps.index}
      >
