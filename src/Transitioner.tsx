...
import { Scene, SceneDescriptorMap, TransitionerLayout, TransitionProps } from './types';
...
class Transitioner extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.positionListener = position.addListener((/* { value } */) => {
      ...
      // value subscription
      //  console.log('position listener', value, this.transitionProps.scene.route.routeName);
    });
  ...
  componentWillUnmount() {
    this.isComponentMounted = false;
    if (this.positionListener) this.state.position.removeListener(this.positionListener);
  }
  ...
  private startTransition(props: Props, nextProps: Props) {
    ...
    const nextScenes = this.computeScenes(props, nextProps);
    ...
    const { isTransitioning } = this.transitionProps.navigation.state;
    ...
    if (!isTransitioning || !indexHasChanged) {
      this.setState(nextState, async () => {
        ...
        if (indexHasChanged) position.setValue(toValue);
        // end the transition
        this.handleTransitionEnd();
      });
      ...
      this.setState(nextState, async () => {
        ...
        const transitionUserSpec = nextProps.configureTransition
          ? nextProps.configureTransition(this.transitionProps, this.prevTransitionProps)
          : null;
        ...
        const positionHasChanged = (position as any).__getValue() !== toValue;
  ...
  private handleTransitionEnd = () => {
    ...
    const { prevTransitionProps } = this;
    this.prevTransitionProps = undefined;
    ...
    this.setState(nextState, async () => {
      if (this.props.onTransitionEnd) {
        const result = this.props.onTransitionEnd(this.transitionProps, prevTransitionProps);
      ...
      if (this.queuedTransition) {
        const { prevProps } = this.queuedTransition;
        this.queuedTransition = null;
...
function buildTransitionProps(props: Props, state: State): TransitionProps {
  ...
  const result = {
    ...
    scene,
    index: scene.index,
    hasPreviousScene: scene.index > 0,
    hasNextScene: scenes.some((otherScene) => otherScene.index > scene.index),
  };
